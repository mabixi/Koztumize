...
def index():
    ...
    path_model = os.path.join(app.config['MODELS'], g.domain, 'model')
    models = {
...
.. meta::
   :model: %s/%s""" % (category, filename)
    ...
    temp_file = NamedTemporaryFile(suffix='.css', delete=True)
    filename = os.path.join(
        app.config['MODELS'], g.domain,
        'model_styles', filename[:-4].replace('_', ' ') + '.css')
...
def stylesheet(path):
    ...
    filename = os.path.join(
        app.config['MODELS'], g.domain, 'model_styles', path + '.css')
    parser = csstyle.Parser([filename])
    text = '/* Generated by CSStyle */\n\n'
    text += repr(parser)
    for engine in csstyle.BROWSERS:
